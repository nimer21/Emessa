    ...
    <div className='bg-half-transparent w-screen fixed nav-item top-0 right-0'>
      <div className='float-right h-screen dark:text-gray-200 bg-white dark:[#484B52] w-400'>
        <div className='flex justify-between items-center p-4 ml-4'>
          <p className='text-xl font-semibold'>Theme Settings</p>
          <button type='button' onClick={() => setThemeSettings(false)} style={{ color: 'rgb(153, 171, 181)', borderRadius: '50%' }} className='text-2xl p-3 hover:drop-shadow-xl hover:bg-light-gray'>
            <MdOutlineCancel />
          </button>
        </div>
            ...
            {themeColors.map((item, index) => (
              <TooltipComponent content={item.name} position='TopCenter' key={index}>
                <div className='relative mt-2 cursor-pointer flex gap-5 items-center'>
                  <button type='button' className='h-10 w-10 rounded-full cursor-pointer hover:drop-shadow-xl' style={{ backgroundColor: item.color }} onClick={() => setColor(item.color)}>
